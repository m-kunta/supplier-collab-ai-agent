...
def compute_asn_insights(
    ...
    *,
    ...
    return {
        ...
        "total_shipped_qty": float(df["qty_shipped_num"].fillna(0.0).sum()),
        "total_received_qty": float(df["qty_received_num"].fillna(0.0).sum()),
        "total_overdue_qty": float(overdue_rows["qty_shipped_num"].fillna(0.0).sum()) if not overdue_rows.empty else 0.0,
