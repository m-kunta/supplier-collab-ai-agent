...
- `src/delivery_attempt_store.py`: SQLite-backed audit log for scheduled notification delivery attempts.
- `src/notification_retry.py`: Channel-level retry/dead-letter orchestration around `NotificationDispatcher`.
...
- `frontend/app/vendors/page.tsx`: Vendor onboarding page for registering suppliers, reviewing registered vendors, and downloading onboarding packs.
...
GOOGLE_CALENDAR_CREDENTIALS_PATH=config/credentials.json
...
When fallback is enabled and OAuth credentials are missing, the scheduler keeps returning mock vendor meetings for local demos.
